...
pub struct HostMetricsManager {
    ...
    bytes_sent_accum: Arc<Mutex<u64>>,
...
impl HostMetricsManager {
    pub fn new() -> Self {
        Self {
            ...
            start_time: None,
            bytes_sent_accum: Arc::new(Mutex::new(0)),
    ...
    pub fn update_rtt(&self, rtt_ms: f32) {
        ...
        if let Some(ref mut net) = m.network.as_mut() {
            // Placeholder / standard fallback if network object structure changes, but since network is not directly option:
            // Wait, in metrics.rs, `SessionMetrics` does not have standard `pub network: NetworkStats` in our custom metrics.rs,
            // oh let's check what our metrics.rs has:
            // control_rtt_ms: Option<f32>,
            // input_rtt_ms: Option<f32>,
    ...
    pub fn update_capture_fps(&self, fps: f32) {
        let mut m = self.metrics.lock().unwrap();
        m.capture_fps = Some(fps);
    }

    pub fn update_capture_latency(&self, latency_ms: f32) {
        let mut m = self.metrics.lock().unwrap();
        m.capture_latency_ms = Some(latency_ms);
    }

    pub fn add_bytes_sent(&self, bytes: usize) {
        let mut accum = self.bytes_sent_accum.lock().unwrap();
        *accum += bytes as u64;
        ...
        if elapsed.as_secs_f32() >= 1.0 {
            ...
            *last_check = Instant::now();
            ...
            let mut m = self.metrics.lock().unwrap();
            let bytes_sec = sent as f64 / elapsed.as_secs_f64();
            m.sent_bytes_per_sec = Some(bytes_sec);
